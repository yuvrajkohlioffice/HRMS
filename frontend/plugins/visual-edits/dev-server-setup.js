...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*HRMS\.sh$/)) {
        return true;
      }

      // Allow all HRMSagent.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*HRMSagent\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        for (const change of variableEditChanges) {
          ...
          if (result.success) {
            ...
            try {
              execSync(`git -c user.name="visual-edit" -c user.email="support@" add "${result.file}"`);
              execSync(`git -c user.name="visual-edit" -c user.email="support@" commit -m "visual_edit_variable_${timestamp}"`);
            } catch (gitError) {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
